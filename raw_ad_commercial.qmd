...
landings_annual <- landings_annual |> 
    mutate(Species = case_match(NMFS.Name,
           ...
           TensofThousands_Pounds = Pounds / 10000)
...
p4 <- landings_seas |> 
    mutate(TensofThousands_Pounds = lbs_headOff / 10000) |> 
    ggplot() + 
    geom_point(aes(x = Year,
                     y = TensofThousands_Pounds,
                   col = Months),
...
-   Annual Landings from NMFS file: both `Pounds` and `Dollars` were read in as character due to commas in the number formatting; parsed these to numeric. Then calculated versions where the numbers will be smaller: `Millions_of_Dollars` and `TensofThousands_Pounds`.  
