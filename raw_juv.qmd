...
Sizes in Oyster Landing Seines were in RCL; here they have been converted to Total Length using the equations from Batchelder et al. 2024, Appendix table A1:  

brown shrimp:  TL = 2.862 + 2.617\*RCL  (R2 = 0.94)  
white shrimp:  TL = 4.271 + 2.43\*RCL   (R2 = 0.95)
...
juv_ol_size <- juv_ol |> 
    ...
    pivot_longer(cols = starts_with("LEN"),
                 names_to = "Rep",
                 values_to = "Length.RCL") |> 
    filter(!is.na(Length.RCL)) |> 
    mutate(Length.TL = case_when(Species == "brown" ~ 2.617*Length.RCL + 2.862,
                                 Species == "white" ~ 2.43*Length.RCL + 4.271,
                                 .default = 999))
```
...
#| fig-cap: "Beeswarm plot of juvenile shrimp length for the Oyster Landing seine data. Lengths represent Total Length, converted from RCL. Points represent measurements of individual shrimp, and are colored by the month in which they were captured. The y-axis represents length. More points spread out along the x-axis for a given length means that there were more individuals of that length captured than regions where points remain closer to the center."

beeswarm_sizeDistn(juv_ol_sizeb,
                   x = "Species", y = "Length.TL", month = "Month",
                   survey = surv_ol) +
    labs(y = "Length (mm)")
```
...
-  Oyster Landing Sizes: converted to TL from RCL using equations from Batchelder et al. 2024 Appendix table A1.  
