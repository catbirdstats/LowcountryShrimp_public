...
There are three SEAMAP files. The only one handled here is the one containing trawl data (weights and total amounts of shrimp). The other two files contain a) additional information from the trawling events; and b) shrimp size data.    
...
-   Two data files are in the `data/raw/ad_seamap` folder and not processed here:  
    -  `SEAMAP_CoastalSurvey_Events_19892022.csv` - contains additional data about each trawl  
    -  `SEAMAP_Sizes.csv` - contains total length data for shrimp from each trawl.  
